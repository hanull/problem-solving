...
public class Solution {
    ...
    static boolean isValidFlag(String[] commandData) {
        String beforeArgumentType = commandData[1];
        for (int index = 1; index < commandData.length; index += 2) {
            String flag = commandData[index];

            if (!flagRules.containsKey(flag) && !ALIAS.containsKey(flag)) return false; // 대응하는 flag가 없는 경우

            if (ALIAS.containsKey(flag)) {
                flag = ALIAS.get(flag);
            }

            // 같은 Argument Type 연속적으로 올 수 없음
            if (index > 1 && beforeArgumentType.equals(flag)) return false;
            else if (!flag.equals("-e")) beforeArgumentType = flag;
            ...
            else if (flagRules.get(flag).equals("STRINGS")) {     // Strings : 1개 이상의 flag argument 확인
                if (index + 1 >= commandData.length) return false;
                int argumentIndex = index + 1;
                while (!commandData[argumentIndex].contains("-")) {
                    if (!isString(commandData[argumentIndex])) return false;
                    argumentIndex++;
                }
                index = argumentIndex;
            }
            ...
            else if (flagRules.get(flag).equals("NUMBERS")) {     // NUMBERS : 1개 이상의 flag argument 확인
                if (index + 1 >= commandData.length) return false;
                int argumentIndex = index + 1;
                while (!commandData[argumentIndex].contains("-")) {
                    if (!isDigit(commandData[argumentIndex])) return false;
                    argumentIndex++;
                }
                index = argumentIndex;
            }
    ...
    static void saveFlagRules(String[] flag_rules) {
        for (String rule : flag_rules) {
            ...
            if (temporarySpace[1].equals("ALIAS")) {
                ALIAS.put(temporarySpace[0], temporarySpace[2]);
            } else {
    ...
    public static void main(String[] args) {
        System.out.println(Arrays.toString(solution("line", new String[]{"-s STRING", "-num NUMBER", "-e NULL", "-n ALIAS -num"}, new String[]{"line -n 100 -s hi -e", "line -n 100 -e -num 150"})));
        System.out.println(Arrays.toString(solution("bank", new String[]{"-send STRING", "-a ALIAS -amount", "-amount NUMBERS"}, new String[]{"bank -send abc -amount 500 200 -a 400", "bank -send abc -a hey"})));
    }
}
